...
export default function TimeSelectionStep({ bookingDetails, onContinue }) {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="w-full p-2 border rounded-lg mb-6" />
                ...
                {availableSlots.length > 0 ? availableSlots.map(slot => {
                    ...
                    return (
                        <button key={slot} onClick={() => setSelectedTime(slot)}
                            className={`p-3 rounded-lg ${isSelected ? 'bg-pink-500 text-white' : 'bg-gray-200'}`}>
                            {time}
                        </button>
                    )
